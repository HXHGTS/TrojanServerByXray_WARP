...
int install_xray() {
    ...
    printf("正在生成强密码. . .\n");
    system("mkpasswd -l 28 > /usr/local/etc/xray/passwd.conf");
    config = fopen("/usr/local/etc/xray/passwd.conf", "r");
    fscanf(config, "%s",passwd);
    fclose(config);
...
int KernelUpdate() {
    if ((fopen("KernelUpdate.sh", "r")) == NULL) {
        ...
        system("curl -sSL https://raw.githubusercontent.com/HXHGTS/Cloudflare_WARP_Connect/main/netflix_support_centos8.sh | sh");
    }
    return 0;
}
